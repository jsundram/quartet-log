...
`report_ambiguity()` adds the complementary check — not "which short forms belong in the table" but "which must not", because an alias maps one name to one person and the sheet may hold several who share it. Three hazards: (1) a bare first name still in the sheet that 2+ full names could match — unfixable by any alias, the rows themselves need editing, since `PLAYER_ALIASES` is keyed on (name, class) and cannot say "this row is Alice Hart and that one is Alice Bek". `attribute_bare_entries` then answers it per ENTRY rather than per name, which is what decides whether there is work to do: two people who share a first name rarely share a stand (circle overlap runs 0.00–0.08 across this log's ambiguous pairs), so the row's other players identify the one who was there. It **must** read `archive/data-raw.csv` — on the processed export `normalizePlayerNames` has already replaced each bare slot name with the alias's own guess, so those rows vote to confirm it and a 0 means nothing; the section says so loudly if handed one, and `audit_all.sh` takes these counts from the raw pass. `fill_forward` then runs the real `fillForward` over that input (via node, empty tables) and returns every row twice, as written and as filled. Only the per-entry evidence lookup reads the filled view — a third of the raw sheet is continuation rows whose players are stated only above — while every index (variant grouping, teammate counts, bare-name counts, `written`, the canonicalized-export detector) is built from the rows **as written**, since each asks what a human typed, and a name typed once in a five-piece session would otherwise clear `MIN_WRITTEN_IN_FULL` five times over. The written row is also the only one with a cell anyone could edit, and it decides the subject's own seat: comparing by name cannot exclude it once fill-forward has expanded that very cell, and the subject then scores as its own stand-mate for whichever rival knows the person it actually is. A cell fill-forward resolves is not reported at all: the app fills before it aliases, so no alias ever sees that cell and there is no hazard to report. If the node bridge fails, `fill_forward` says so **on stdout** and `audit_all.sh` repeats it: a degraded run inflates NEEDS MEMORY with rows whose answer sits one line above, and that must not read as a finding. `candidate_index` is the single definition of who a bare name could be, shared with the per-name list so the two cannot disagree — class-keyed like `PLAYER_ALIASES`, excluding one-letter surnames (`Peter O` is Peter Ouyang abbreviated, not a rival), with circles built only from names typed out in full. A verdict needs evidence on both sides: `MIN_WRITTEN_IN_FULL` appearances for the winner and for any alias it contradicts, plus at least one rival the sheet has actually named — the people most often logged bare are the ones whose full name is rarest, so an unnamed rival's silence means "never seen", not "not them", and every such rival is printed as one that could not be ruled out. Findings split four ways: the alias is contradicted, the room resolves an entry no alias covers (the app counts that bare form as a separate person — mechanical), nobody has decided (no alias, no evidence: the only decaying kind), and the rest counted. On the real log that is 2, 1 and 0; (2) an existing alias keyed on such a name, which silently resolves every future bare entry to whichever person the table names; (3) an alias whose canonical name appears nowhere in the sheet *and* isn't what any sheet name resolves to — usually a spelling fix applied to the data but not to the table, reported with a `did you mean:` hint from the same first token. The liveness test matters: without it, every alias doing its job (its target is absent from the raw sheet by design) reads as needing repair. Hazards 1 and 2 can only fire once a second full name with that first name exists in the data.
