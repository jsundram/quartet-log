...
Per-year stats column shows five numbers (Pieces, Unique Pieces, People played with, Playing Days, Longest Streak) at `cellSize*2` through `cellSize*6`, with tooltips wired via `attachStatTooltip` (works on hover and tap). The stat values + tooltip copy live in `_yearStatDefs`, shared by both layouts (horizontal right-hand column and fullscreen below-grid rows) so they can't drift.

The Longest Streak value comes from `longestPlayingStreak(days)` (exported from `src/calendarComponent.js` so it's unit-testable). It reads adjacency **positionally** — `days[i+1]` IS the next calendar day — which is only valid because the array is the gap-free ascending run `d3.timeDay.range` produces and `d3.groups` preserves per year. Feed it a sparse array and it silently overstates streaks by closing real gaps. `dataProcessor`'s `longestConsecutiveRun` is the sparse-input equivalent (adjacency inferred from day ordinals); the two algorithms are pinned to one definition of a streak by a cross-check test that runs the same span through both. The legend SVG is sized to exactly `10 * cellSize` wide and uses CSS `width: min(170px, 17%)` + `margin-left: min(40.5px, 4.05%)` so it tracks the calendar grid's first 10 cells across all viewport widths.
