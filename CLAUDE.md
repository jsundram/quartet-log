...
`report_ambiguity()` adds the complementary check — not "which short forms belong in the table" but "which must not", because an alias maps one name to one person and the sheet may hold several who share it. Three hazards: (1) a bare first name still in the sheet that 2+ full names could match — unfixable by any alias, the rows themselves need editing, since `PLAYER_ALIASES` is keyed on (name, class) and cannot say "this row is Alice Hart and that one is Alice Bek". `attribute_bare_entries` then answers it per ENTRY instead of per name, which is what decides whether there is work to do: two people who share a first name rarely share a stand (teammate-circle overlap runs 0.00–0.08 across the log's ambiguous pairs), so the other players in the row identify the one who was there long after anyone could recall the evening. It **must** read `archive/data-raw.csv`: on the processed export `normalizePlayerNames` has already replaced each bare slot name with whatever the alias guessed, so those rows have joined the guessed person's circle and vote to confirm it — a wrong alias confirms itself, only `Others?` entries can ever disagree, and a 0 means nothing. It prints a loud caveat when it detects that input, and `audit_all.sh` takes these counts from the raw pass for the same reason. Circles are likewise built only from names someone wrote out in full: a typed name is evidence, an alias-supplied one is the hypothesis. Two guards keep it from sending you to "fix" correct data — a candidate whose last token is a single letter is an abbreviation (`Peter O`) rather than a rival person, and a circle counts as evidence only above `MIN_WRITTEN_IN_FULL` explicit appearances, since the people most often logged bare are exactly the ones whose full name is rarest, so their non-match means "never seen named", not "not them". Findings split four ways: the room contradicts the alias (wrong person credited today), the room resolves an entry no alias covers (the app counts the bare form as a separate person — mechanical), nobody has decided (no alias, no evidence: the only ones that decay), and the rest counted, including entries with an alias standing that the run has no evidence to second-guess. On the real log that is 2, 1 and 0 against 455 counted; (2) an existing alias keyed on such a name, which silently resolves every future bare entry to whichever person the table names; (3) an alias whose canonical name appears nowhere in the sheet *and* isn't what any sheet name resolves to — usually a spelling fix applied to the data but not to the table, reported with a `did you mean:` hint from the same first token. The liveness test matters: without it, every alias doing its job (its target is absent from the raw sheet by design) reads as needing repair. Hazards 1 and 2 can only fire once a second full name with that first name exists in the data.
