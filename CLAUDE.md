...
- `DateFilterWidget` (`src/dateFilterWidget.js`) — reusable segmented date range picker (`All` / `YTD` / `1Y` / `6M` / `1M` / `Custom`). Class-based selectors scoped to mount point so multiple instances can coexist; Home and Dashboard each have their own, so a new range option appears on both for free. `1M` starts at the same day of the previous month via the pure exported `monthsAgo`, which clamps to that month's last day when it is shorter (Mar 31 → Feb 29/28) instead of letting `setMonth` overflow forward into March.
