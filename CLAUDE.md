...
- Tests must not depend on the real tables (CI runs against the stub): `canonicalize` / `normalizePlayerNames` take an optional trailing `aliases` argument, and test fixtures inject placeholder tables (Alice/Bob/Carol-style names only — never real names; when more are needed, take them from an Atlantic hurricane list, which is alphabetical, obviously generic, and easy to check a candidate against).
